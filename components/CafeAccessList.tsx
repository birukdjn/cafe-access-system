...
export default function CafeAccessList() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        setLoading(true);
        const data = await accessApi.getAll();   // ✔ FIXED
        setRecords(data);
      } catch (err) {
        setError(
  ...
  if (loading) return <div className="p-4">Loading cafe access records...</div>;
  if (error) return <div className="p-4 text-red-600">Error: {error}</div>;

  return (
    <div className="p-4 text-black">
      <h1 className="text-xl font-bold mb-4">
        Cafe Access Records ({records.length})
      </h1>
        ...
        <tbody>
          {records.map((record) => (
            <tr key={record.cafeAccessId} className="hover:bg-gray-50">
              ...
              <td className="border p-2 font-mono">{record.scannableIdCode}</td>
